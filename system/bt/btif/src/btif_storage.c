...
/*******************************************************************************
**
** Function         btif_storage_add_aam_app_device
**
** Description      BTIF storage API - Adds the newly authorised device-app pair
**                   to NVRAM along with the permission level
**
** Returns          BT_STATUS_SUCCESS if the store was successful,
**                  BT_STATUS_FAIL otherwise
**
...
bt_status_t btif_storage_add_aam_app_device(char *app_dev,
                                           uint8_t perm_level)
{
    int ret = btif_config_set_int(app_dev, "PermissionLevel", (int)perm_level);

    BTIF_TRACE_WARNING("CROSSAPPBLE (not warning) %s: Adding %s with permission %d", __FUNCTION__, app_dev, perm_level);
...
/*******************************************************************************
**
** Function         btif_storage_load_aam_app_dev_pairs
**
** Description      BTIF storage API - Loads all the app-dev pairs from NVRAM
**                  and adds to the BTA.
**
** Returns          BT_STATUS_SUCCESS if successful, BT_STATUS_FAIL otherwise
**
*******************************************************************************/
bt_status_t btif_storage_load_aam_app_dev_pairs(void)
{
    bool retstatus;

    for (const btif_config_section_iter_t *iter = btif_config_section_begin(); iter != btif_config_section_end(); iter = btif_config_section_next(iter)) {
        const char *app_dev_id = btif_config_section_name(iter);

        BTIF_TRACE_WARNING("CROSSAPPBLE (not a warning) Test potential app-dev pair:%s", app_dev_id);
        if (!string_is_app_dev_pair(app_dev_id))
            continue;

        BTIF_TRACE_WARNING("CROSSAPPBLE (not a warning) App-dev pair tested ok:%s", app_dev_id);
        int perm_level = -1;
            retstatus = btif_config_get_int(app_dev_id,
                                        "PermissionLevel", &perm_level);
        BTIF_TRACE_WARNING("CROSSAPPBLE (not a warning) App-dev pair:%s has permission level %d", app_dev_id, perm_level);
        
        // Split app_dev_id into app_id and dev_id.
        char* temp_app_dev_id = strdup(app_dev_id);
        const char* app_id = strtok(temp_app_dev_id, "-");
        const char* dev_id = strtok(NULL, "-");
        
        BTIF_TRACE_WARNING("CROSSAPPBLE (not a warning) App-dev pair split into :%s, %s", app_id, dev_id);
        retstatus = BTA_Add_AppDevRecord(app_id, dev_id, perm_level);
    }
    return BT_STATUS_SUCCESS;
}

bool string_is_app_dev_pair(const char *string) {
    // If there is no hyphen, return NULL.
    char* pPosition = strchr(string, '-');
    if (pPosition == NULL) {
        return false;
    }
    
    char* temp_app_dev_id = strdup(string);
    const char* app_id = strtok(temp_app_dev_id, "-");
    const char* dev_id = strtok(NULL, "-");
    app_id = NULL;

    if (!string_is_bdaddr(dev_id)) {
        return false;
    }
    return true;
}