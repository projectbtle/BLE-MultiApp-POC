...
/* aam */
extern btaam_interface_t *btif_aam_get_interface();
#endif
/* avrc target */
...
static const void* get_aam_interface ()
{
    LOG_INFO("CROSSAPPBLE", "/system/bt/btif/src/bluetooth.c=>get_aam_interface");

    /* sanity check */
    if (interface_ready() == FALSE)
        return NULL;
    
    return btif_aam_get_interface();
}
